...
def train(rank, args, chkpt_path, hp, hp_str):
    print('train________________________')
    if args.num_gpus > 1:
    ...
    if chkpt_path is not None:
        ...
        if rank == 0:
            logger.info("Starting new training run._______________________")
    ...
    for epoch in range(init_epoch, hp.train.epochs):
        ...
        log_file_path = "testlog/test.txt"  # 指定日志文件的路径
        os.makedirs(os.path.dirname(log_file_path), exist_ok=True)  # 确保文件夹存在
        ...
        for ppg, ppg_l, vec, pit, spk, spec, spec_l, audio, audio_l in loader:
            ...
            pit = pit.to(device)
            spk = spk.to(device)
            print(f'spk shape:{spk.shape}________________________________________')
            logger.info(f'spk shape:{spk.shape}________________________________________')
            with open(log_file_path, 'a') as log_file:
                log_message = f'spk shape: {spk.shape} ________________________________________\n'
                print(log_message.strip())  # 在控制台输出
